...
const parseTestingInput = (testing: string) => {
  ...
  switch (testingLower) {
    ...
    case "true":
      throw new Error(
        "Setting 'TESTING' to 'true' or 'false' is deprecated, use 'auto', 'never' or 'always'.",
      );
    ...
    default:
      throw new Error(
        `Unexpected value for input 'TESTING': '${TESTING}', needs to be one of 'auto', 'never' or 'always'.`,
      );
  }
};
...
const getAsset = (
  ...
  const helper = (
    ...
    if (release === undefined) {
      throw new Error(
        `A release with the tag '${version}' could not be found.`,
      );
    }
    ...
    core.info(
      `Finding assets matching the platform, architecture and ${doTesting ? "marked as testing" : "not marked as testing"}.`,
    );
    ...
    if (assetsMatchingFilters.length === 0) {
      throw new Error(
        `Release '${release.tag_name}' has no assets matching the platform and architecture '${platformAndArchitecture}' and ${doTesting ? "marked as testing" : "not marked as testing"}.`,
      );
    }
